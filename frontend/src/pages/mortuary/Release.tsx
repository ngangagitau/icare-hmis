...
const generateReleaseId = () => `REL-${Date.now().toString().slice(-6)}`;

const MortuaryRelease = () => {
  const [releaseId] = useState(() => generateReleaseId());
      ...
      <Card>
        ...
        <CardContent>
          <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={(e) => { e.preventDefault(); toast.success("Body released successfully"); }}>
            <div className="space-y-2"><Label>Release ID</Label><Input value={releaseId} readOnly className="bg-slate-50 text-slate-700" /></div>
