...
const generateDonorId = () => `DON-${Date.now().toString().slice(-6)}`;

const DonorRegistration = () => {
  const [donorId] = useState(() => generateDonorId());
      ...
      <Card>
        ...
        <CardContent>
          <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={(e) => { e.preventDefault(); toast.success("Donor registered successfully"); }}>
            <div className="space-y-2"><Label>Donor ID</Label><Input value={donorId} readOnly className="bg-slate-50 text-slate-700" /></div>
